...
const autoEvalSignupFields: FormField[] = [
  {
    name: "name",
    label: "Full Name",
    type: "text",
...
export default function AutoEvalSignup({ onSuccess, onValidationError }: AutoEvalSignupProps) {
  const [isLoading, setIsLoading] = useState(false);
  const navigate = useNavigate();
  const { setProfile } = useProfile();

  const handleGoogleSignup = () => {
    window.location.href = `${import.meta.env.VITE_API_URL}/v1/autoeval/auth/google`;
  };

  const handleSubmit = async (data: Record<string, string | string[]>) => {
    setIsLoading(true);
    try {
      const response = await apiClient.post('/v1/autoeval/register', data);
