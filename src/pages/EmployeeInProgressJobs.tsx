...
export default function EmployeeInProgressJobs() {
  const { profile } = useProfile()
  const navigate = useNavigate();

  const handleJobAction = (action: string, row: Record<string, unknown>) => {
    if (action === "view") {
      const url = `/employee/jobs/${row.id}?source=in-progress`;
      navigate(url);
    ...
    <EmployeeLayout>
      <div className="flex flex-col gap-6 pb-8">
        <div className="flex items-center justify-between px-8 lg:px-16">
          <div>
            <h1 className="text-3xl font-bold">In Progress Jobs</h1>
            <p className="text-muted-foreground">
              Track your ongoing job applications and work progress
            </p>
          </div>
        </div>

        <RecommendedJobsView
          endpoint="/v1/employee/jobs/IN-PROGRESS"
          dataKey="jobs"
          title=""
